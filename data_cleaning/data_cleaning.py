...
data = pd.read_csv("/Users/toshalpatel/is5152_project/concatenated_listings.csv", usecols=imp_cols_list)
...
data['amenities_n'] = data['amenities'].apply(lambda x : str(x).replace('{','').replace('}','')
                        .replace('"','').lower().replace('nan','').split(',')).head()
...
print(dict(Counter(amenities).most_common(5)))


# Hence, {'wifi': 5, 'kitchen': 5, 'heating': 5, 'smoke detector': 5, 'carbon monoxide detector': 5} are the TOP 5 amentities 
...
df['is_carbon_mo_detector'] = data['amenities_n'].apply(lambda x : if_amenities(x,'carbon monoxide detector'))
